...
def calculateY(nextStatesBatch, rewardBatch, doneBatch, gamma, model):
    graph = model.graph
    evalNetOutput_ = graph.get_collection_ref('evalNetOutput')[0]
    states_ = graph.get_collection_ref("states")[0]
    evalNetOutputBatch = model.run(evalNetOutput_, feed_dict={states_: nextStatesBatch})
    yBatch = []
...
class BuildModel:
    ...
    def __call__(self, layersWidths, summaryPath="./tbdata"):
        ...
        with graph.as_default():
            ...
            initWeight = tf.random_uniform_initializer(-1, 1)
                ...
                with tf.variable_scope("trainEvalHiddenLayers"):
                    ...
                    for i in range(len(layersWidths)):
                        fcLayer = tf.layers.Dense(units=layersWidths[i], activation=tf.nn.relu, kernel_initializer=initWeight,
                                                  bias_initializer=initBias, name="fcEvalHidden{}".format(i+1), trainable = True)
                    ...
                    evalHiddenOutput_ = tf.identity(activation_, name="outputHidenEval")
                    outputEvalFCLayer = tf.layers.Dense(units=self.numActionSpace, activation=tf.nn.relu, kernel_initializer=initWeight,
                                                    bias_initializer=initBias, name="fcEvalOut{}".format(len(layersWidths)+1), trainable = True)
...
class TrainOneStep:
    ...
    def __init__(self, batchSize, updateFrequency, learningRate, gamma):
    ...
    def __call__(self, model, miniBatch, batchSize):
        ...
        yBatch = calculateY(nextStatesBatch, rewardBatch, doneBatch, self.gamma, model)
...
class InitializeReplayBuffer:
    ...
    def __init__(self, reset, transit, getReward, isTerminal, actionDim):
        self.reset = reset
...
def upgradeEpsilon(epsilon):
    epsilon = epsilon + 0.01*(1-0.5)
    return epsilon


class ForwardOneStep:

    def __init__(self, transit, getReward, isTerminal):
        self.transit = transit
        self.getReward = getReward
        self.isTerminal = isTerminal
...
class RunTimeStep:
    ...
    def __call__(self, states, trajectory, model, replayBuffer, score):
        ...
        for i in range(self.actionDelay):
            ...
            nextStates, reward, done = self.forwardOneStep(states, action)
            replayBuffer = memorize(replayBuffer, states, action, nextStates, reward, done, self.actionDim)
